...
class CBAM3DMASK(nn.Module):
    def __init__(self, planes):
        super(CBAM3DMASK,self).__init__()
...
class CBAMLSTMCell(nn.Module):
    ...
    def __init__(self, input_size, input_dim, hidden_dim, kernel_size, bias):
        ...
        super(CBAMLSTMCell, self).__init__()
        ...
        # self.conv = nn.Conv3d(in_channels=self.input_dim ,
        #                       out_channels=4 * self.hidden_dim,
        #                       kernel_size=self.kernel_size,
        #                       padding=self.padding,
        #                       bias=self.bias)
        self.conv = nn.Sequential(
            nn.Conv3d(in_channels=self.input_dim ,
                    out_channels=2 * self.hidden_dim,
                    kernel_size=self.kernel_size,
                    padding=self.padding,
                    bias=self.bias),
            CBAM3DMASK(2 * self.hidden_dim)
        )
    def forward(self, input_tensor, cur_state):

        h_cur, c_cur = cur_state

        combined = torch.stack([input_tensor, h_cur], dim=2) # b,c,2,h,w

        combined_conv = self.conv(combined) # b,c4,2,h,w
        b,c,n,h,w = combined_conv.shape
        combined_conv = combined_conv.view(b,-1,h,w)
...
class CDAMLSTM(nn.Module):

    def __init__(self, input_size, input_dim, hidden_dim, kernel_size, num_layers,
                 batch_first=False, bias=True, return_all_layers=False):
        super(CDAMLSTM, self).__init__()
        ...
        for i in range(0, self.num_layers):
            ...
            cell_list.append(CDAMLSTMCell(input_size=(self.height, self.width),
...
class CDAMBLSTM(nn.Module):
    ...
    def __init__(self, input_size, input_dim, hidden_dim,
        ...
        super(CDAMBLSTM, self).__init__()
        self.forward_net = CDAMLSTM(input_size, input_dim, hidden_dims//2, kernel_size,
                                    num_layers, batch_first=batch_first, bias=bias,
                                    return_all_layers=return_all_layers)
        self.reverse_net = CDAMLSTM(input_size, input_dim, hidden_dims//2, kernel_size,
                                    num_layers, batch_first=batch_first, bias=bias,
                                    return_all_layers=return_all_layers)
